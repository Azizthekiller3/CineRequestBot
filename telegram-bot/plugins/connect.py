...
async def _owner_check(bot, message, group):
    ...
    if message.from_user.id != group.get("user_id"):
        await message.reply(f"Only {html.escape(group.get('user_name','the owner'))} can use this command.")
        return False
    return True


async def _resolve_channel(bot, user_client, ch_id: int):
    """Try to get channel title. Returns (title, ok)."""
    chat = await get_chat_safe(bot, ch_id)
    if chat:
        return getattr(chat, "title", str(ch_id)), True
    if user_client:
        try:
...
async def connect_cmd(bot, message):
    ...
    try:
        ...
        user_client = User if User.is_connected else None
    except Exception:
        user_client = None

    title, _ = await _resolve_channel(bot, user_client, ch_id)
...
async def connections_cmd(bot, message):
    ...
    channels = group.get("channels", [])
    if not channels:
        return await message.reply("📭 No channels connected yet.\nUse /connect to add one.")
...
async def disconnect_cb(bot, update):
    ...
    # format: discon_{group_id}_{ch_id}
    if len(parts) < 3:
    ...
    channels = group.get("channels", [])
    if ch_id not in channels:
        return await update.answer("Already removed.", show_alert=True)

    channels.remove(ch_id)
    await update_group(group_id, {"channels": channels})

    # Rebuild the connections list in the message
    if not channels:
        try:
    ...
    for i, cid in enumerate(channels, 1):
        title, _ = await _resolve_channel(bot, user_client, cid)
